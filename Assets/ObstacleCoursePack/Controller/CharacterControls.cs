...
public class CharacterControls : MonoBehaviourPunCallbacks {
	...
	public GameObject cam;//GameObject.Find("playercam" + GameObject.
	...
	void  Start (){
		...
		//targetindex = transform.name.Replace("playername", "");
		cam = GameObject.Find("Camera Holder(Clone)");//"Camera Holder(Clone)"
	...
	void FixedUpdate()
	{
			...
			if (canMove)
			{
				if (moveDir.x != 0 || moveDir.z != 0 && photonView.IsMine)
				{
